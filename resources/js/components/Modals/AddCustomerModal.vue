<template>
    <div
        v-if="show"
        class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4"
    >
        <div
            class="bg-gray-800 rounded-2xl w-full max-w-md border border-gray-700 shadow-2xl overflow-hidden animate-fade-in-up"
        >
            <div
                class="p-4 border-b border-gray-700 flex justify-between items-center bg-gray-800"
            >
                <h3 class="text-lg font-bold text-white">Add New Customer</h3>
                <button
                    @click="$emit('close')"
                    class="text-gray-400 hover:text-white transition-colors"
                >
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        class="h-6 w-6"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                    >
                        <path
                            stroke-linecap="round"
                            stroke-linejoin="round"
                            stroke-width="2"
                            d="M6 18L18 6M6 6l12 12"
                        />
                    </svg>
                </button>
            </div>

            <form @submit.prevent="submit" class="p-4 space-y-4">
                <div class="space-y-1">
                    <label class="text-sm font-medium text-gray-400"
                        >Name</label
                    >
                    <input
                        v-model="form.name"
                        type="text"
                        required
                        class="w-full bg-gray-700 border-gray-600 rounded-lg text-white focus:ring-blue-500 focus:border-blue-500 placeholder-gray-500"
                        placeholder="Customer Name"
                    />
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1">
                        <label class="text-sm font-medium text-gray-400"
                            >Phone</label
                        >
                        <input
                            v-model="form.phone"
                            type="tel"
                            required
                            class="w-full bg-gray-700 border-gray-600 rounded-lg text-white focus:ring-blue-500 focus:border-blue-500 placeholder-gray-500"
                            placeholder="01xxxxxxxxx"
                        />
                    </div>
                    <div class="space-y-1">
                        <label class="text-sm font-medium text-gray-400"
                            >Backup Phone</label
                        >
                        <input
                            v-model="form.backup_phone"
                            type="tel"
                            class="w-full bg-gray-700 border-gray-600 rounded-lg text-white focus:ring-blue-500 focus:border-blue-500 placeholder-gray-500"
                            placeholder="Optional"
                        />
                    </div>
                </div>

                <div class="space-y-1">
                    <label class="text-sm font-medium text-gray-400"
                        >Address</label
                    >
                    <textarea
                        v-model="form.address"
                        rows="2"
                        class="w-full bg-gray-700 border-gray-600 rounded-lg text-white focus:ring-blue-500 focus:border-blue-500 placeholder-gray-500"
                        placeholder="Full Address..."
                    ></textarea>
                </div>

                <div class="space-y-1">
                    <label class="text-sm font-medium text-gray-400"
                        >Special Mark</label
                    >
                    <input
                        v-model="form.special_mark"
                        type="text"
                        class="w-full bg-gray-700 border-gray-600 rounded-lg text-white focus:ring-blue-500 focus:border-blue-500 placeholder-gray-500"
                        placeholder="e.g. Near the mosque"
                    />
                </div>

                <div class="pt-2">
                    <button
                        type="submit"
                        class="w-full bg-green-600 hover:bg-green-500 text-white font-bold py-2.5 rounded-xl shadow-lg shadow-green-900/20 transition-all flex justify-center items-center gap-2"
                    >
                        <span>Save Customer</span>
                    </button>
                </div>
            </form>
        </div>
    </div>
</template>

<script setup>
import { ref, reactive } from "vue";

const props = defineProps({
    show: Boolean,
});

const emit = defineEmits(["close", "save"]);

const form = reactive({
    name: "",
    phone: "",
    backup_phone: "",
    address: "",
    special_mark: "",
});

const submit = () => {
    // In a real app, validation and API call here
    emit("save", { ...form });
    // Reset form or handle success
    form.name = "";
    form.phone = "";
    form.backup_phone = "";
    form.address = "";
    form.special_mark = "";
    emit("close");
};
</script>

<style scoped>
@keyframes fade-in-up {
    0% {
        opacity: 0;
        transform: translateY(20px);
    }
    100% {
        opacity: 1;
        transform: translateY(0);
    }
}
.animate-fade-in-up {
    animation: fade-in-up 0.3s ease-out forwards;
}
</style>
