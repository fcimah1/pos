<template>
    <header class="bg-gray-800 text-white p-2 flex justify-between items-center px-6 shadow-md z-50 overflow-x-auto select-none" dir="rtl">
        <div class="flex items-center gap-6 shrink-0">
            ...
            <!-- روابط التنقل -->
            <nav class="flex items-center bg-gray-900/50 backdrop-blur-md rounded-2xl p-1 gap-1 border border-gray-700/30">
                <button 
                    @click="$emit('go-to', '/pos')" 
                    class="px-5 py-2 rounded-xl text-sm font-bold transition-all whitespace-nowrap flex items-center gap-2"
                ...
                <button 
                    @click="$emit('go-to', '/admin/products')" 
                    ...
                    :class="currentPath.includes('/admin/products') ? 'bg-gray-800 text-white' : ''"
                >
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg>
                    المنتجات
                </button>
                <button 
                    @click="$emit('go-to', '/admin/categories')" 
                    class="px-5 py-2 rounded-xl text-sm font-bold transition-all hover:bg-gray-800 text-gray-400 hover:text-white whitespace-nowrap flex items-center gap-2"
                    :class="currentPath.includes('/admin/categories') ? 'bg-gray-800 text-white' : ''"
                >
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2z" /></svg>
                    الأصناف
                </button>
                <button 
...
defineEmits(['go-to', 'logout', 'open-driver-settlement']);
</script>
